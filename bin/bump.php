...
$response = json_decode(file_get_contents(
    'https://api.github.com/repos/http-request-handler-interop/fig-standards/tags',
    false,
    stream_context_create([
        'http' => [
                'method' => 'GET',
                'header' => [
                        'User-Agent: http-request-handler-interop',
                ],
        ],
    ])
));
...
if ($semversion === 'latest') {
    ...
    foreach ($response as $tagInfo) {
        if (preg_match(
            '/http-request-handler-(?P<version>\d+.\d+.\d+)/',
            $tagInfo->name,
            $m
        )) {
            $version = $m['version'];
    ...
    foreach ($response as $tagInfo) {
        if ($tagInfo->name === 'http-request-handler-'.$semversion) {
...
foreach ($composerJson->repositories as $repo) {
    ...
    if ($package->name === 'php-fig/fig-standards') {
        ...
        $package->source->reference = 'http-request-handler-'.$semversion;
    }
}
